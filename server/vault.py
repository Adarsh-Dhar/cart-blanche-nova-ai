...
========================================
Purpose:
  - Takes the total budget from `cart_mandate` and encrypts it using the
    SKALE BITE v2 threshold encryption protocol (mock in dev, real in prod).
  - Stores the result in `encrypted_budget` for audit/privacy purposes.
  - This node is intentionally silent — it does not emit a user-facing message
    because it runs automatically between merchant approval and settlement
    awaiting the MetaMask signature.
...
async def vault_node(state: AgentState) -> dict:
    ...
    try:
        ...
        logger.info(
            "[Vault] Budget %d USDC units encrypted via SKALE BITE v2 "
